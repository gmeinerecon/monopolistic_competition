// Local, no-cloud multiplayer server for Monopolistic Competition game
// Run with: node server.js
// Requires Node >= 18. No external cloud services. Uses only built-in http and 'ws' for WebSocket.

const http = require('http');
const fs = require('fs');
const path = require('path');
const WebSocket = require('ws');

const PORT = process.env.PORT || 8080;

// ---- Static file server (no Express) ----
const server = http.createServer((req, res) => {
  let filePath = req.url;
  if (filePath === '/' || filePath === '') filePath = '/host.html';
  if (filePath === '/host') filePath = '/host.html';
  if (filePath === '/student') filePath = '/student.html';
  const fullPath = path.join(__dirname, 'public', decodeURI(filePath));

  // Prevent path traversal
  if (!fullPath.startsWith(path.join(__dirname, 'public'))) {
    res.writeHead(403); res.end('Forbidden'); return;
  }

  fs.readFile(fullPath, (err, content) => {
    if (err) {
      res.writeHead(404); res.end('Not found'); return;
    }
    const ext = path.extname(fullPath).toLowerCase();
    const types = {
      '.html': 'text/html; charset=utf-8',
      '.js': 'text/javascript; charset=utf-8',
      '.css': 'text/css; charset=utf-8',
      '.json': 'application/json; charset=utf-8'
    };
    res.writeHead(200, {'Content-Type': types[ext] || 'text/plain; charset=utf-8'});
    res.end(content);
  });
});

// ---- WebSocket state ----
const wss = new WebSocket.Server({ server });

const palette = ['#2563eb','#16a34a','#dc2626','#f59e0b','#06b6d4','#a855f7','#f97316','#ec4899','#10b981','#3b82f6','#ef4444'];

// Random utilities (Box–Muller)
function randn() {
  let u = 0, v = 0;
  while (u === 0) u = Math.random();
  while (v === 0) v = Math.random();
  return Math.sqrt(-2.0 * Math.log(u)) * Math.cos(2.0 * Math.PI * v);
}

function safeNum(v, def=0){ const n = Number(v); return Number.isFinite(n) ? n : def; }

const defaultParams = {
  baseDemand: 100,
  brandLiftMarket: 4,
  priceDragMarket: 3,
  betaBrand: 0.8,
  betaPrice: 0.6,
  brandPersistence: 0.8,
  fixedCost: 10,
  variableCost: 2,
  convexCost: 0.05,
  brandCostLinear: 1.5,
  brandCostQuadratic: 0.2,
  shockMean: 0,
  shockStd: 10
};

const state = {
  round: 1,
  params: { ...defaultParams },
  firms: {}, // id -> {id, name, color, brandStock, wsId, connected}
  choices: {}, // id -> {price, quantity, brand, submitted}
  history: [],
  nextFirmId: 1,
  hostClients: new Set(), // WebSocket instances acting as host dashboards
  studentClients: new Map(), // ws -> firmId
};

function broadcast(msg) {
  const data = JSON.stringify(msg);
  wss.clients.forEach(ws => {
    if (ws.readyState === WebSocket.OPEN) ws.send(data);
  });
}

function send(ws, msg){ if(ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify(msg)); }

function getPublicState() {
  // Build a minimal state to send to clients
  const firmsArray = Object.values(state.firms).map(f => ({
    id: f.id, name: f.name, color: f.color, brandStock: +(f.brandStock||0).toFixed(2), connected: !!f.connected
  }));
  return {
    round: state.round,
    params: state.params,
    firms: firmsArray,
    choices: state.choices,
    history: state.history
  };
}

function resetGame(hard = false) {
  state.round = 1;
  state.history = [];
  if (hard) {
    // HARD RESET: remove all firms and choices
    for (const [ws, fid] of state.studentClients.entries()) {
      try { send(ws, { type: 'kicked' }); } catch {}
      try { ws.close(); } catch {}
    }
    state.studentClients.clear();
    state.firms = {};
    state.choices = {};
    state.nextFirmId = 1;
  } else {
    // SOFT RESET: keep roster but reset brand and choices
    Object.values(state.firms).forEach(f => { f.brandStock = 5; });
    state.choices = {};
    Object.values(state.firms).forEach(f => {
      state.choices[f.id] = { price: 10, quantity: 20, brand: 5, submitted: false };
    });
  }
}

function computeResults() {
  const P = state.params;
  const firmList = Object.values(state.firms);
  if (firmList.length === 0) return null;

  // Effective brand this round
  const effectiveBrand = {};
  firmList.forEach(f => {
    const ch = state.choices[f.id] || { price:0, quantity:0, brand:0 };
    const bInv = Math.max(0, safeNum(ch.brand));
    const prev = safeNum(f.brandStock, 0);
    effectiveBrand[f.id] = P.brandPersistence * prev + bInv;
  });

  // Averages
  let avgPrice = 0, avgBrandEff = 0;
  firmList.forEach(f => {
    avgPrice += safeNum(state.choices[f.id]?.price);
    avgBrandEff += effectiveBrand[f.id];
  });
  avgPrice /= firmList.length;
  avgBrandEff /= firmList.length;

  // Demand baseline & shock
  const demandBaseline = P.baseDemand + P.brandLiftMarket * avgBrandEff - P.priceDragMarket * avgPrice;
  const shock = P.shockStd > 0 ? (P.shockMean + P.shockStd * randn()) : P.shockMean;
  const marketDemand = Math.max(0, demandBaseline + shock);

  // Utilities & shares (logit)
  const utils = firmList.map(f => {
    const p = safeNum(state.choices[f.id]?.price);
    const B = effectiveBrand[f.id];
    return P.betaBrand * B - P.betaPrice * p;
  });
  const maxU = Math.max(...utils);
  const expU = utils.map(u => Math.exp(u - maxU));
  const denom = expU.reduce((a,b)=>a+b, 0) || 1;

  const firmOutcomes = firmList.map((f, idx) => {
    const ch = state.choices[f.id] || { price:0, quantity:0, brand:0 };
    const p = Math.max(0, safeNum(ch.price));
    const q = Math.max(0, Math.floor(safeNum(ch.quantity)));
    const bInv = Math.max(0, safeNum(ch.brand));
    const B = effectiveBrand[f.id];
    const share = expU[idx] / denom;
    const demanded = share * marketDemand;
    const sales = Math.min(q, demanded);
    const fixed = P.fixedCost;
    const prodCost = P.variableCost * q + P.convexCost * q * q;
    const brandCost = P.brandCostLinear * bInv + P.brandCostQuadratic * bInv * bInv;
    const cost = fixed + prodCost + brandCost;
    const revenue = p * sales;
    const profit = revenue - cost;
    const leftover = Math.max(0, q - sales);
    return {
      id: f.id,
      name: f.name,
      p: +p.toFixed(2), q: q, bInvest: +bInv.toFixed(2), bEff: +B.toFixed(2),
      share, demanded, sales, leftover,
      cost: +cost.toFixed(2), revenue: +revenue.toFixed(2), profit: +profit.toFixed(2)
    };
  });

  const totalSales = firmOutcomes.reduce((a,o)=>a+o.sales,0);
  const avgProfit = firmOutcomes.reduce((a,o)=>a+o.profit,0)/firmList.length;

  const results = {
    avgPrice: +avgPrice.toFixed(2),
    avgBrandEff: +avgBrandEff.toFixed(2),
    demandBaseline: +demandBaseline.toFixed(2),
    shock: +shock.toFixed(2),
    marketDemand: +marketDemand.toFixed(2),
    totalSales: +totalSales.toFixed(2),
    avgProfit: +avgProfit.toFixed(2),
    firmOutcomes,
  };

  // Append to history
  state.history.push({
    round: state.round,
    summary: {
      avgPrice: results.avgPrice,
      avgBrandEff: results.avgBrandEff,
      demandBaseline: results.demandBaseline,
      shock: results.shock,
      marketDemand: results.marketDemand,
      totalSales: results.totalSales,
      avgProfit: results.avgProfit,
    },
    firmOutcomes: results.firmOutcomes.map(x=>({ ...x }))
  });

  // Carry brand forward
  firmList.forEach(f => { f.brandStock = effectiveBrand[f.id]; });

  // Next round
  state.round += 1;

  return results;
}

function csvFromHistory(){
  const headers = [
    'round','firmId','firmName','price','quantity','brand_invest','brand_effective','share','demanded','sales','leftover','revenue','cost','profit','avg_price','avg_brand_eff','demand_baseline','shock','market_demand','total_sales','avg_profit'
  ];
  const rows = [headers.join(',')];
  state.history.forEach(h => {
    const s = h.summary;
    h.firmOutcomes.forEach(o => {
      const r = [
        h.round,
        o.id,
        JSON.stringify(o.name||''),
        o.p,
        o.q,
        o.bInvest,
        o.bEff,
        (o.share).toFixed(4),
        o.demanded.toFixed(2),
        o.sales.toFixed(2),
        o.leftover.toFixed(2),
        o.revenue.toFixed(2),
        o.cost.toFixed(2),
        o.profit.toFixed(2),
        s.avgPrice,
        s.avgBrandEff,
        s.demandBaseline,
        s.shock,
        s.marketDemand,
        s.totalSales,
        s.avgProfit
      ];
      rows.push(r.join(','));
    });
  });
  return rows.join('\n');
}

wss.on('connection', (ws) => {
  ws.isAlive = true;
  ws.on('pong', () => ws.isAlive = true);

  ws.on('message', (data) => {
    let msg = {};
    try { msg = JSON.parse(data); } catch { return; }
    const t = msg.type;

    if (t === 'join') {
      if (msg.role === 'host') {
        state.hostClients.add(ws);
        send(ws, { type: 'state', payload: getPublicState() });
      } else if (msg.role === 'student') {
        const name = String(msg.name||'Firm '+state.nextFirmId).slice(0,60);
        const id = state.nextFirmId++;
        const color = palette[(id-1) % palette.length];
        state.firms[id] = { id, name, color, brandStock: 5, wsId: Date.now()+':'+Math.random().toString(36).slice(2), connected: true };
        state.choices[id] = state.choices[id] || { price: 10, quantity: 20, brand: 5, submitted: false };
        state.studentClients.set(ws, id);
        // welcome
        send(ws, { type: 'joined', payload: { firmId: id, color } });
        broadcast({ type: 'state', payload: getPublicState() });
      }
      return;
    }

    if (t === 'set_params') {
      // host only
      if (!state.hostClients.has(ws)) return;
      state.params = { ...state.params, ...(msg.params||{}) };
      broadcast({ type: 'state', payload: getPublicState() });
      return;
    }

    if (t === 'reset_game') {
      if (!state.hostClients.has(ws)) return;
      const hard = !!msg.hard;
      resetGame(hard);
      broadcast({ type: 'state', payload: getPublicState() });
      return;
    }

    if (t === 'choice_update') {
      // student updating their inputs
      const fid = state.studentClients.get(ws);
      if (!fid) return;
      const ch = state.choices[fid] || { price: 0, quantity: 0, brand: 0, submitted: false };
      ch.price = safeNum(msg.choice?.price, ch.price);
      ch.quantity = Math.max(0, Math.floor(safeNum(msg.choice?.quantity, ch.quantity)));
      ch.brand = Math.max(0, safeNum(msg.choice?.brand, ch.brand));
      ch.submitted = false;
      state.choices[fid] = ch;
      broadcast({ type: 'state', payload: getPublicState() });
      return;
    }

    if (t === 'choice_submit') {
      const fid = state.studentClients.get(ws);
      if (!fid) return;
      const ch = state.choices[fid] || { price: 0, quantity: 0, brand: 0, submitted: false };
      ch.submitted = true;
      state.choices[fid] = ch;
      broadcast({ type: 'state', payload: getPublicState() });
      return;
    }

    if (t === 'kick_firm') {
      if (!state.hostClients.has(ws)) return;
      const id = Number(msg.id);
      if (!Number.isFinite(id) || !state.firms[id]) { send(ws,{type:'error',message:'Unknown firm id'}); return; }
      // notify and disconnect the student socket controlling this firm
      for (const [client, fid] of Array.from(state.studentClients.entries())) {
        if (fid === id) {
          try { send(client, { type: 'kicked' }); } catch {}
          try { client.close(); } catch {}
          state.studentClients.delete(client);
        }
      }
      delete state.firms[id];
      delete state.choices[id];
      broadcast({ type: 'state', payload: getPublicState() });
      return;
    }

    if (t === 'compute_round') {
      if (!state.hostClients.has(ws)) return;
      const results = computeResults();
      if (results) {
        broadcast({ type: 'results', payload: { results, state: getPublicState() } });
      } else {
        send(ws, { type: 'error', message: 'No firms connected.' });
      }
      return;
    }

    if (t === 'export_csv') {
      if (!state.hostClients.has(ws)) return;
      const csv = csvFromHistory();
      send(ws, { type: 'csv_data', payload: { csv } });
      return;
    }

  });

  ws.on('close', () => {
    // Remove from host set
    if (state.hostClients.has(ws)) state.hostClients.delete(ws);
    // If student, mark disconnected but keep firm in roster
    const fid = state.studentClients.get(ws);
    if (fid) {
      state.studentClients.delete(ws);
      if (state.firms[fid]) state.firms[fid].connected = false;
      broadcast({ type: 'state', payload: getPublicState() });
    }
  });
});

// Heartbeat to clean up dead sockets
setInterval(() => {
  wss.clients.forEach(ws => {
    if (!ws.isAlive) return ws.terminate();
    ws.isAlive = false; ws.ping();
  });
}, 30000);

server.listen(PORT, () => {
  console.log(`Local multiplayer server running on http://localhost:${PORT}`);
  console.log('Host dashboard: /host  |  Student console: /student');
});
